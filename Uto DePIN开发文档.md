...
2. 容差升值： 系统为用户产出的价值增加1%的容差，用于升值，即允许价值在一定范围内波动而不影响奖励发放。
...
买入价格
[(对付储备金池÷全网用户持有流通数量包含LP池子里面代币、不含主代币池、不含黑洞地址)+LP流动性池价值÷1.9]+每枚代币设定的容差=买入的价格

卖出价格
[(对付储备金池÷全网用户持有流通数量包含LP池子里面代币、不含主代币池、不含黑洞地址)+LP流动性池价值÷1.9]-LP池子深度越高容差越低兑付能力越高\容差越低=卖出的价格]

卖出最低价值为
对付储备金池÷全网用户持有流通数量包含LP池子里面代币、不含主代币池、不含黑洞地址=最低价值

自动计算买入和卖出的权重计算流程：
买入流程
1. 初始买入权重
所有买入资金初始时均进入兑付储备金池。
公式：W_{\text{buy, initial}} = 100\%
2. 流动性增加后的买入权重
随着流动性的增加，部分资金将进入LP流动性池。
公式：
W_{\text{buy, reserve}} = 100\% - L
W_{\text{buy, LP}} = L
其中 L 表示用户向LP流动性池添加的流动性比例。

买入操作：
资金首先根据初始买入权重全部进入兑付储备金池。
随着流动性的增加，资金将按照 W_{\text{buy, reserve}} 和 W_{\text{buy, LP}} 的比例分别进入兑付储备金池和LP流动性池。
代币从LP流动性池和代币池转移到用户钱包。

卖出流程
1. 初始卖出权重
所有卖出资金初始时均从兑付储备金池中扣除。
公式：W_{\text{sell, initial}} = 100\%
2. 流动性增加后的卖出权重
随着流动性的增加，部分资金将从LP流动性池中扣除。
公式：
W_{\text{sell, reserve}} = 100\% - L
W_{\text{sell, LP}} = L
其中 L 表示用户向LP流动性池添加的流动性比例。
交易流程概述
卖出操作：
代币从用户钱包转入代币池。
资金首先根据初始卖出权重全部从兑付储备金池扣除。
随着流动性的增加，资金将按照 W_{\text{sell, reserve}} 和 W_{\text{sell, LP}} 的比例分别从兑付储备金池和LP流动性池扣除。
资金从LP流动性池和兑付储备金池转移到用户钱包。

自定义买入卖出权重调整器
运行原理：
允许用户设置初始的买入和卖出权重，其中80%至99%的资金进入兑付储备金池，剩余的20%至1%进入LP流动性池。随着用户向LP流动性池添加更多的流动性，系统将自动调整权重，以确保流动性的增加得到适当的反映。如果自动计算的权重超过用户自定义的权重，系统将采用自动计算的权重。
计算公式：
1. 初始买入权重设置：
用户自定义的兑付储备金池权重：W_{\text{buy, custom}}
用户自定义的LP流动性池权重：W_{\text{buy, LP, custom}} = 100\% - W_{\text{buy, custom}}
2. 初始卖出权重设置：
用户自定义的兑付储备金池权重：W_{\text{sell, custom}}
用户自定义的LP流动性池权重：W_{\text{sell, LP, custom}} = 100\% - W_{\text{sell, custom}}
3. 流动性增加后的买入权重自动计算：
兑付储备金池权重：W_{\text{buy, reserve}} = 100\% - L
LP流动性池权重：W_{\text{buy, LP}} = L
其中 L 表示用户向LP流动性池添加的流动性比例。
4. 流动性增加后的卖出权重自动计算：
兑付储备金池权重：W_{\text{sell, reserve}} = 100\% - L
LP流动性池权重：W_{\text{sell, LP}} = L
5. 最终权重确定：
如果 W_{\text{buy, reserve}} \geq W_{\text{buy, custom}}，则使用 W_{\text{buy, reserve}} 和 W_{\text{buy, LP}}。
否则，使用 W_{\text{buy, custom}} 和 W_{\text{buy, LP, custom}}。
如果 W_{\text{sell, reserve}} \geq W_{\text{sell, custom}}，则使用 W_{\text{sell, reserve}} 和 W_{\text{sell, LP}}。
否则，使用 W_{\text{sell, custom}} 和 W_{\text{sell, LP, custom}}。
买入操作示例：
假设用户设置初始买入权重为90%进入兑付储备金池，10%进入LP流动性池。用户向LP流动性池添加流动性，比例 L 增加到20%。
1. 初始买入权重：
W_{\text{buy, custom}} = 90\%
W_{\text{buy, LP, custom}} = 10\%
2. 流动性增加后的买入权重自动计算：
W_{\text{buy, reserve}} = 100\% - 20\% = 80\%
W_{\text{buy, LP}} = 20\%
3. 最终权重确定：
由于 W_{\text{buy, reserve}}（80%）小于 W_{\text{buy, custom}}（90%），系统将使用用户自定义的权重：
买入权重：90%进入兑付储备金池，10%进入LP流动性池。
4. 买入操作：
用户有10000元资金进行买入操作。
9000元进入兑付储备金池。
1000元进入LP流动性池。
相应数量的代币从LP流动性池和代币池转移到用户钱包。
卖出操作示例：
假设用户设置初始卖出权重为90%从兑付储备金池中扣除，10%从LP流动性池中扣除。用户向LP流动性池添加流动性，比例 L 增加到30%。
1. 初始卖出权重：
W_{\text{sell, custom}} = 90\%
W_{\text{sell, LP, custom}} = 10\%
2. 流动性增加后的卖出权重自动计算：
W_{\text{sell, reserve}} = 100\% - 30\% = 70\%
W_{\text{sell, LP}} = 30\%
3. 最终权重确定：
由于 W_{\text{sell, reserve}}（70%）小于 W_{\text{sell, custom}}（90%），系统将使用自动计算的权重：
卖出权重：70%从兑付储备金池扣除，30%从LP流动性池扣除。
4. 卖出操作：
用户有10000元代币进行卖出操作。
7000元从兑付储备金池扣除。
3000元从LP流动性池扣除。
相应数量的资金从LP流动性池和兑付储备金池转移到用户钱包。
通过这个自定义买入卖出权重调整器，用户可以灵活地设置初始权重，并随着流动性的增加自动调整权重，确保交易的公平性和流动性的合理分配。
