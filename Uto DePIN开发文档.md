...
4. 价格调整机制：
当代币价格下跌超过40%时，系统将根据兑付储备金池的全网用户持有流通数量包含LP池子里面代币、不含主代币池、不含黑洞地址，来重新定价代币。
运行公式：
1. 购买代币计算：
...
3. 价格调整计算：
设 S 为代币的市场价格，C 为代币的初始价格。
当 S < 0.6C 时，触发价格调整机制。
调整后的价格 S' 根据兑付储备金池的资金量 R 和全网流通量 CIRC 计算，S' = \frac{R}{CIRC}。
总结：
这种机制通过结合流动性池和兑付储备金池，旨在维持代币价格的稳定性，同时为用户提供流动性和价格发现机制。通过允许初始兑付储备金为零，系统在启动时对资金的要求较低，但依赖于流动性池来提供初始的稳定性和流动性。价格调整机制确保了在市场压力下，代币价格能够反映市场供需关系，增强了系统的适应性和稳定性。
...
在流动性池和兑付储备金池的模型中，买入和卖出权重的计算涉及到资金流入和流出的分配。这里提供一个概念性的框架来计算这些权重，而不涉及具体数值。
买入权重计算：
1. 定义变量：
T：流动性池中的代币数量。
U：流动性池中的资金数量（以U为单位）。
P：兑付储备金池中的代币数量。
k：流动性池的常数乘积，即 k = T \times U。
2. 初始状态：
流动性池的初始状态为 T 枚代币和 U U。
兑付储备金池的初始状态为 P 枚代币。
3. 买入操作：
用户投入 X U进行买入操作。
4. 计算流动性池的分配：
用户投入的资金将按照流动性池中代币和资金的比例进行分配。
5. 买入权重：
流动池的买入权重 W_L 为： W_L = \frac{U}{U + \frac{T}{1.01}}
兑付储备金池的买入权重 W_R 为： W_R = \frac{T}{U + \frac{T}{1.01}}
6. 考虑价格容差：
由于每枚代币的价格要考虑1%的容差，实际的买入权重可能会有所调整。
卖出权重计算：
1. 卖出操作：
用户卖出 Y 枚代币。
2. 计算流动性池的分配：
用户卖出的代币将按照流动性池中代币和资金的比例进行分配。
3. 卖出权重：
流动池的卖出权重 W_L' 为： W_L' = \frac{U}{U + T}
兑付储备金池的卖出权重 W_R' 为： W_R' = \frac{T}{U + T}

结论：
通过上述步骤，我们可以计算出用户在买入和卖出操作中，资金在流动池和兑付储备金池之间的分配权重。这些权重反映了用户操作对两个池的影响，以及每个池对用户操作的响应。具体的权重计算需要根据流动池和兑付储备金池的实时数据进行。这种方法提供了一个概念性的框架，用于理解和计算在考虑价格容差的情况下，资金如何在不同池之间分配。
