...
买入最低价值
对付储备金池÷全网用户持有流通数量包含LP池子里面代币、不含主代币池、不含黑洞地址+每枚代币设定的容差=买入的最低价值
...
自定义买入卖出权重调整器(添加LP流动池生效)

运行原理：
用户首先设定一个固定的权重比例，其中1%至20%的资金进入LP流动性池，而剩余的99%至80%进入兑付储备金池。这个初始固定权重将用于第一次交易。随后，系统将根据兑付储备金的实际值自动调整权重，以确保交易的公平性和流动性的合理分配。
计算公式：
1. 初始固定权重：
用户设定的LP流动性池初始权重：W_{\text{LP, initial}}（1%至20%）
用户设定的兑付储备金池初始权重：W_{\text{reserve, initial}} = 100\% - W_{\text{LP, initial}}（99%至80%）
2. 流动性增加后的自动计算权重：
兑付储备金池权重：W_{\text{reserve, auto}} = \frac{R}{R + L} \times 100\%
LP流动性池权重：W_{\text{LP, auto}} = \frac{L}{R + L} \times 100\%
其中 R 表示兑付储备金的值，L 表示LP流动性池的值。
3. 最终权重确定：
对于后续的买入和卖出操作，系统将使用自动计算的权重 W_{\text{reserve, auto}} 和 W_{\text{LP, auto}}。
买入操作示例：
假设用户初始设定15%的资金进入LP流动性池，85%进入兑付储备金池。用户随后向LP流动性池添加流动性，使得LP流动性池的值 L 增加，而兑付储备金的值 R 保持不变。
1. 初始固定权重：
W_{\text{LP, initial}} = 15\%
W_{\text{reserve, initial}} = 85\%
2. 流动性增加后的自动计算权重：
假设 R = 10000，L = 2000，则 W_{\text{reserve, auto}} = \frac{10000}{10000 + 2000} \times 100\% \approx 83.33\%
W_{\text{LP, auto}} = \frac{2000}{10000 + 2000} \times 100\% \approx 16.67\%
3. 买入操作：
用户有10000元资金进行买入操作。
约8333元进入兑付储备金池。
约1667元进入LP流动性池。
...
卖出操作示例：
假设用户初始设定15%的资金从LP流动性池中扣除，85%从兑付储备金池中扣除。用户随后从LP流动性池中提取流动性，使得LP流动性池的值 L 减少，而兑付储备金的值 R 增加。
1. 初始固定权重：
W_{\text{LP, initial}} = 15\%
W_{\text{reserve, initial}} = 85\%
2. 流动性减少后的自动计算权重：
假设 R = 12000，L = 1000，则 W_{\text{reserve, auto}} = \frac{12000}{12000 + 1000} \times 100\% \approx 92\%
W_{\text{LP, auto}} = \frac{1000}{12000 + 1000} \times 100\% \approx 8\%
3. 卖出操作：
用户有10000元代币进行卖出操作。
约9200元从兑付储备金池扣除。
约800元从LP流动性池扣除。
相应数量的资金从LP流动性池和兑付储备金池转移到用户钱包。
通过这个基于兑付储备金的动态权重调整器，用户的初始交易将遵循其设定的固定比例，而后续交易将根据兑付储备金的实际值自动调整，以适应市场变化和流动性需求。
